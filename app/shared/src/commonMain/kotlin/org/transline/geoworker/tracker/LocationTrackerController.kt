...
class LocationTrackerController(
    ...
    fun saveLocationConfiguration(
        ...
        if (storage.isRegistrationLocked()) {
            ...
            val sameEndpoint =
                normalizeCoordinatesEndpoint(apiEndpoint) == storage.getApiEndpoint()
    ...
    /**
     * Точка входа для платформенного continuous GPS.
     * Троттлинг по updateIntervalMinutes, lock isRequestInProgress, backoff при ошибке.
     */
    ...
    suspend fun completeTripAfterModeration() {
        ...
        if (lastLocation != null) {
            ...
            if (success) {
                listeners.forEach {
                    it.onLocationSent(lastLocation.latitude, lastLocation.longitude, lastLocation.timestampMs)
                }
            } else {
                listeners.forEach {
                    it.onLocationFailed("Ошибка отправки (сохранено в офлайн-очередь)")
    ...
    /**
     * Clears trip schedule + unlocks registration. Keeps endpoint/uuid/auth/offline queue.
     * Full wipe remains [TrackingStorage.clear] for logout / explicit reset.
     */
    fun clearTripState() {
        storage.setTrackingActive(false)
        storage.clearLastSentTimestamp()
    ...
    fun saveSecureConfig(
        ...
        return try {
            ...
            listeners.forEach {
                it.onSecureConfigEvent(SecureConfigEventType.KEYCHAIN_SAVED, null)
            }
            true
        } catch (e: Exception) {
            listeners.forEach {
                it.onSecureConfigEvent(SecureConfigEventType.KEYCHAIN_ERROR, e.message?.take(200))
            }
            false
        }
    }
    ...
    fun clearSecrets() {
        secureStore.clear()
        listeners.forEach {
            it.onSecureConfigEvent(SecureConfigEventType.KEYCHAIN_CLEARED, null)
        }
    }

    /**
     * KMP-only configurable HTTP probe (D-02, D-07, D-09).
     * Emits status + short message + method + redacted URL — no response body, no tokens.
     * No automatic 401→refresh→retry.
     */
    suspend fun httpProbe(url: String, method: String, body: String? = null): HttpProbeResult {
        ...
        if (!HttpProbe.isAllowedMethod(normalized)) {
            return emitHttpFailed(normalized, safeUrl, status = null, message = "HTTP method not allowed")
        }
        ...
        if (access.isNullOrEmpty()) {
            ...
            return emitHttpFailed(normalized, safeUrl, status = null, message = "Access token missing")
        }
    ...
    private fun probeHttpMethod(method: String): HttpMethod = when (method) {
        "GET" -> HttpMethod.Get
    ...
    /**
     * Persist auth when provided; otherwise keep existing or fall back to deprecated default (D-07).
     * Prefer explicit [authHeader] / RN WithAuth / WithBearer — default is client-shipped Basic debt.
     */
